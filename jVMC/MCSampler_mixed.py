...
class MCSampler_mixed:
    ...
    def __init__(self, net, sampleShape, key, updateProposer=None, numChains=1, updateProposerArg=None,
                 numSamples_A=100,numSamples_M=100, thermalizationSweeps=10, sweepSteps=10, initState=None, mu=2, logProbFactor=0.5):
        ...
        self.numSamples_A = numSamples_A
        self.numSamples_M = numSamples_M
        self.numSamples = numSamples_A + numSamples_M
    ...
    def sample(self, parameters=None, numSamples=None, multipleOf=1):
        ...
        if numSamples is None:
            ...
            numSamples_A = self.numSamples_A
            numSamples_M = self.numSamples_M
        else:
            numSamples_A = numSamples//2
            numSamples_M = numSamples -numSamples_A
        #if self.net.is_generator:
        #jax.debug.print("{y}, {x}",y=numSamples_A,x=numSamples_M)
        if parameters is not None:
            tmpP = self.net.params
            self.net.set_parameters(parameters)
        #jax.debug.print("{y} {x}", y=self.net.parameters,x= numSamples_A,)

        configs_A = self._get_samples_gen(self.net.parameters, numSamples_A, multipleOf)
        configs_M, _ = self._get_samples_mcmc(parameters, numSamples_M, multipleOf)
        #jax.debug.print("{y} {x}", y=configs_A.shape,x=configs_M.shape)
        configs = jnp.concatenate((configs_A,configs_M),axis=1)
