...
class AdaptiveHeun_ref_logs:
    ...
    def step(self, t, f, y, sampler,normFunction=jnp.linalg.norm,  **rhsArgs):
        """ This function performs an integration time step.
        ...
        while fe < 1.:
            ...
            # compute deviation
            #s1, logpsi1, p1 = sampler.sample(parameters = yInitial+dy1)
            #s0, logpsi0, p0 = sampler.sample(parameters = yInitial+dy0)
            sampler.psi.set_parameters(yInitial+dy1)
            logpsi1 = sampler.psi(s_ref)
            sampler.psi.set_parameters(yInitial+dy0)
            logpsi0 = sampler.psi(s_ref)
            sampler.psi.set_parameters(yInitial)

            updateDiff = normFunction(logpsi0- logpsi1)
