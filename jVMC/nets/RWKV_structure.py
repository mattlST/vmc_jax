...
class CpxRWKV_2d(nn.Module):
    ...
    def setup(self):
        ...
        if self.patch_size>1:
            raise ValueError("Patch size bigger 1 is not implemented yet! (for 2d)")
        ...
        self.gate = nn.Dense(self.embedding_size, use_bias=False, param_dtype=self.dtype,
                        kernel_init=nn.initializers.variance_scaling(self.init_variance,mode="fan_in",distribution="truncated_normal"))
    ...
    def forward_with_state(self, s: Array, block_states: Array = None,  output_state: bool = False) -> Array:
        ...
        if self.one_hot:
            ...
            if output_state:
                ...
                s = self.index_map(s.reshape(self.PL,self.patch_size))
                yH = jnp.pad(s[:-1],(1,0),mode='constant',constant_values=0)
                yH = self.embed(yH)
                #yH *= expand_dims(jnp.concatenate((jnp.zeros(1,dtype=int),jnp.ones(self.L-1,dtype=int))),0)
                #jax.debug.print("{x}",x= yH.shape)
                #sV = jnp.roll(s,self.L1,axis=0)
                #sV *= jnp.concatenate((jnp.zeros(self.L1,dtype=int),jnp.ones(self.L-self.L1,dtype=int)))
                #yV = self.embed(sV)
                yV = jnp.pad(s[:-self.L1],(self.L1,0),mode='constant',constant_values=0)
                yV = self.embed(yV)
                #yV *= expand_dims(jnp.concatenate((jnp.zeros(self.L1,dtype=int),jnp.ones(self.L-self.L1,dtype=int))),0)

                #sV *= expand_dims(jnp.concatenate((jnp.zeros(self.L1,dtype=int),jnp.ones(self.L-self.L1,dtype=int))),-1)
                #yV += expand_dims(jnp.concatenate((jnp.full(self.L1,0,dtype=int),jnp.zeros(self.L-self.L1,dtype=int))),-1)
        ...
        if block_states is None:
            block_statesH= repeat(None)
            block_statesV= repeat(None)
            if output_state:                
                if len(s)>1: # case when we have particle conservation
                    #yH *= expand_dims(jnp.concatenate((jnp.zeros(1,dtype=int),jnp.ones(self.L-1,dtype=int))),0)
                    sV = jnp.pad(s[:1-self.L1],(self.L1-1,0),mode='constant',constant_values=0)
                    yV = self.embed(sV)
                    #yV *= expand_dims(jnp.concatenate((jnp.zeros(self.L1,dtype=int),jnp.ones(self.L-self.L1,dtype=int))),0)
            ...
            block_statesH= block_states[-1][0]
            block_statesV= block_states[0][1]
            sV = block_states[0][2]
            yV = self.embed(sV) * block_states[0][3] #expand_dims(self.embed(sprev[0]), 0)

        for blockH,blockV,stateH,stateV in zip(self.blockHs,self.blockVs, block_statesH,block_statesV):
            yH, new_stateH = blockH(yH, stateH)
            yV, new_stateV = blockV(yV, stateV)
            
            if output_state:
                next_statesH.append(new_stateH)
                next_statesV.append(new_stateV)
                
        if block_states is None:
            next_states = [([empty_state(self.num_heads,self.embedding_size) for i in range(self.num_layers)],
                            [empty_state(self.num_heads,self.embedding_size) for i in range(self.num_layers)],s,0) 
                            for _ in range(self.L1-2)] + [(next_statesH,next_statesV,s,1)]
        else:
            next_states = block_states[1:] +[(next_statesH,next_statesV,s,1)]
            #y = self.gate(jnp.concatenate((yV,yH)))
    ...
    def sample(self, numSamples: int, key) -> Array:
        ...
        def generate_sample(key):
            ...
            logits, carry = self(jnp.full(1,0,dtype=int),block_states = None, output_state=True)
